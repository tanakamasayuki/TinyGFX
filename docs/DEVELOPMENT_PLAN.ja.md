...
| **P2** | 文書と実装の同期 | 継続。**2026-08-29 に Q1〜Q13 の決着（D26〜D32）まで反映済み** |
...
**同じ種類の間違いを、その後もう一度やった**（2026-08-29、D27）。
`drawRaw565` を窓経路に変えたとき、**クリップをコアから復号器へ移した**ため、
`clipX0 - x` を 16bit で計算して x = -32768 で桁溢れし、
**画像データの手前を読んで 1,050,624 画素を送った。**

教訓は「32bit にする」ではなく、**クリップを自分でやる経路を作ったら、
そのたびに極端な座標のテストを足す**こと。`tests/clip/` と同じケースを
`tests/image_fmt/` にも置いた。
...
**無い。2026-08-29 で Q1〜Q13 は全部決着した。** 最後まで残っていた 7 件
（Q1 糖衣クラス、Q2 色定数、Q4 `setSwapBytes`、Q6 PROGMEM 画像、
Q10 コールバックの形、Q11 基準機の移行時期、Q12 UTF-8）は、
すべて実測してから決めた（D26〜D32）。
