...
| # | 論点 | なぜ先か |
| --- | --- | --- |
| Q8 | ライブラリ名 `TinyGFX` の重複確認 | **公開をブロックする**（[EXTERNAL_REQUESTS.ja.md](EXTERNAL_REQUESTS.ja.md) E6） |
| Q9 | 「使う文字だけ埋め込む」をどちらが持つか | LGFXFontToolJs への依頼内容が変わる（E4） |

Q7 は実測で決着した（Panel の virtual は 7 本）。
それ以外（糖衣クラスの範囲、色定数のプレフィクス、`setSwapBytes` など）は、
**実物を見て決めたほうが早い。**
