...
| [E22](#e22) | host-arduino-core | ~~汎用の拡張点（ライフサイクル・時計口・UART）~~ **解決。1.7.0 で公開** | — | しない |
...
## E22. host-arduino-core — 汎用の拡張点（ライフサイクル・時計口・Serial 注入） {#e22}

**2026-09-03 起票。** 検証基盤（[docs/emu/](emu/README.ja.md)）の土台になるが、
**依頼内容はすべて Protocol を知らない汎用機能**。単体でも「ホスト実行の
拡張点が増える」だけの一般的な改善になる形で頼む。

### 方針の変更を先に 1 つ

以前の設計（docs/emu/ の初版）は「既存フックを廃止して作り直す」と書いていたが、
**廃止しない方針に変えた。** 観測と応答の分離・多重化は上物（中間層）の仕事で、
ホストコアのタップは**単スロット・Arduino 語彙のままで正しい**と分かったため。
既存の利用者（TinyM5Board のトレース等）も壊れない。

**実現方式はどれも指定しない。** 登録式でも weak 関数の上書きでも、他の形でも
よい —— 下の性質が満たされれば依頼としては満足。方式まで縛るのは
こちらの都合の押し付けになるため、書くのは**要件（能力と性質）だけ**にする。

### 依頼 1 — ライフサイクルの介入点（4 点）

`setup()` の直前・直後、毎周の `loop()` の直前・直後に、外部のコードを
差し込める点が欲しい（現状の thunk: `setup(); while { loop(); … }`）。

満たしてほしい性質:

- 呼ばれる**位置と順序**が上の 4 点で固定であること
- **利用者は 1 者で足りる**（多重化はこちら側の仕事。単スロットでも
  weak 上書きでもよい、はこの性質の言い換え）
- 何も差さないときの挙動が現状と同じであること

### 依頼 2 — 時間アクセスの一本化と、その 1 点の差し替え

やってほしいことは 2 段。

1. **一本化**: いま各所で `steady_clock` を直接参照している時間処理を、
   共通の 2 関数（「いま」を返す・「待つ」）の経由に集約する。対象は
   `millis` / `micros` / `delay` / `delayMicroseconds` に加えて、**内部の
   タイムアウトすべて**（実測: `Stream.h:25-44` の `readBytes` が
   `steady_clock` + `sleep_for` を直に使っている）
2. **差し替え**: その 2 関数を外から上書きできるようにする（weak 関数でも
   登録式でも、方式は任意）

満たしてほしい性質:

- 「いま」だけでなく**「待ち」も丸ごと**その関数に委譲されること
  （上書きした側が待ちの中で自分の処理を刻めるため。ここが要点）
- 上書きしなければ現状の実時間実装のまま（挙動不変）
- ~~`steady_clock` の直参照が残らないこと（1 か所でも残ると、上書きしたとき
  そこだけ時間が進まず**永久に満了しない**タイムアウトになる）~~
  **← この主張は過大だった（2026-09-03、先方の全数調査で訂正）。** ハングするのは
  「**仮想の時計を期限に読みながら、その時計を進めない待ち方をする**」箇所だけ。
  期限も待ちも実時間なら（長さがずれるだけで）満了はする。よって一本化の対象は
  4 つの時間関数 + `Stream::timedRead/timedPeek` で足りる


### 依頼 3 — Serial の受信注入と、デバイス用 UART

- **`Serial` の受信にプログラムから積める**こと（いまは stdin/ソケット系のみ。
  実測: `SerialClass::available()` → `HostArduino::serialAvailable()`）。
  積み方の形式は問わない
- できれば **`Serial1` / `Serial2` のインスタンス**（GPS・モデムのような
  「デバイスが繋がる UART」。コンソールの `Serial` と分離したい）。
  こちらは優先度低で、段階を分けてよい

### 何に使うか（参考。依頼自体はこれに依存しない）

検証基盤の Arduino 中間層が、この 4 点 + 時計口の上に載る:
preSetup/postSetup = 試験の支度と初期ダンプ、preLoop = 進行役と ISR、
postLoop = 時刻の歩進、時計口 = 仮想時間（`delay` だらけのテストが実時間で
眠らなくなる）。詳細は [docs/emu/ENVIRONMENT.ja.md](emu/ENVIRONMENT.ja.md)。

### 2026-09-03: 先方の調査回答 —— スコープ合意

全数調査つきの回答が来た。**こちらの見落としを 2 つ拾っている。**

1. **thunk は 2 か所**（通常ボードと `NATIVE_USE_SDL2`）。片方だけだと lgfx /
   display ボードで介入点が消える —— 両方に入れてもらえる
2. **「直参照が残ると永久に満了しない」は過大**（上の訂正）。
   `HTTPClient::waitData` は期限が `millis()`（仮想）でも待ちが `delay(1)`
   （仮想）なので自力で進んで満了する。**必要十分条件は「期限の時計と待ちの
   時計が一致していること」**で、`steady_clock` の grep は必要条件にすらならない
   （`condition_variable::wait_for` は grep に写らない、という先方の注記も正しい）

**合意した実施範囲:**

| 依頼 | 実施 |
| --- | --- |
| 1. ライフサイクル 4 点 | そのまま（thunk 2 か所とも）。登録式で可（方式は任意のまま） |
| 2. 時計口 | 「いま」+「1 スライス待つ」の 2 関数。**delay のループ本体・`runtimePoll`・停止チェックはコアに残す**（上書き側が忘れられない設計 —— 良い判断）。経由は 4 時間関数 + `Stream::timedRead/timedPeek` |
| 3. UART | **新クラス + `Serial1`/`Serial2`。`Serial`（コンソール）は無変更**。tx はキューで取り出し、rx は積み口。`HardwareSerial` 互換は取らない（独立の Stream 派生。既知の制限として記録: `HardwareSerial&` で受けるライブラリには渡せない。要るとき再検討） |

「onTick と待ち上書きは同一機構」という先方の指摘は、こちらが waitSlice を
時計口に畳んだ結論と同じ —— 両側で独立に同じ答えになったので、この形は固い。

**未確定 3 件へのこちらの回答:**

1. **postLoop と `runtimePoll` の順序 → 案 A**（`preLoop → loop → postLoop →
   runtimePoll`）。理由: 取り込みが**時刻歩進の後**だと、外部入力はつねに
   「次の周のもの」としてスタンプされ、周の因果がトレース上で閉じる。
   案 B だと「アプリが走り終わった周」に入力イベントが混ざり、アプリが
   見られなかったものがその周に写る
2. **仮想時間下のビジーウェイト → こちら（上書き側）の仕事**。コアは何も
   しなくてよい。中間層に「読み取りごと微小加算」の opt-in を持ち、既定は
   進めない + 境界として文書化（docs/emu の既定方針どおり）
3. **対象外から戻すものは無し**。ソケット系の実時間維持はむしろ必須
   （pytest が実時間で繋ぐ）。1 つだけ追加のお願い: **「実時間のまま残る
   タイムアウトの一覧」**（TLS 5 秒予算・FreeRTOS cv 等）を README に
   置いてほしい —— 仮想時計で書くテスト作者が「ここは仮想と一致しない」を
   引ける場所が要るため

### 2026-09-03: 実装のリリース前確認 —— 合格

実装（未リリース）を検査した。**合意との差異なし + 良い追加が 2 つ。**

- 合意どおり: 案 A の順序（`kPostLoop` → `runtimePoll`）、thunk 2 か所、
  `delay` のループ本体・`runtimePoll`・停止チェックはコア残留、
  `Stream::timedRead/timedPeek` が時計口経由、`Serial` 無変更、
  実時間残留タイムアウトの一覧が README に載った
- 良い追加: **`yield()` が wait(0) として時計口を通る**（ビジーウェイト系への
  芽）。**`clockRealNowMicros()`**（実時計の別口 —— 「実時間で速い」の証明に使える）

先方の全スイート **20 passed**。加えて**受け入れ試作** `tests/runtime/accept_emu_tick`
を書いて回した —— docs/emu の tick モデル（仮想時計 + wait スライス = tick、
kPreLoop = 進行役、kPostLoop = 歩進）を 3 つの口の上でそのまま演じるもの。
アプリ半分は仮想時間を知らない普通のデバウンス + AT コマンドのコード。

| 検証 | 結果 |
| --- | --- |
| GPIO 注入で作ったボタン押下を、アプリ自身のデバウンスが検出 | `PRESS held_ms=20`（台本どおり 20 仮想 ms） |
| AT → OK を同一 `loop()` 周内で往復 | `at_reply_ok=1`、`at_virtual_ms=1`（タイムアウト 100ms の 1/100） |
| `delay(5000)` の代金 | 仮想 5000ms・**実時間 1 秒未満**（`clockRealNowMicros` で証明） |

**1 passed。リリース可の判断。** 受け入れ試作は先方リポジトリに未コミットで
置いてある —— 3 つの口を同時に使う統合テストとして採用してもよいし、
不要なら消してよい旨を伝える。

### 2026-09-03: 1.7.0 としてリリース

受け入れ試作 `tests/runtime/accept_emu_tick` は**汎用化のうえ採用された**
（`director_in_delay` —— delay の中で進行役が回った回数の検証などが追加）。
検証基盤の段階 2（Arduino ポート = host バインディング）の土台がこれで揃った。
