...
| D | 12,060 | +6,168 | 6,400 | 572 | +68 | 96 | OK |
| E | 12,680 | **+6,788** | 7,200 | 572 | +68 | 96 | OK |
| T | 13,668 | +7,776 | 8,400 | 1,128 | +624 | 700 | OK |
| P1 | 12,340 | +6,448 | 参考 | 584 | +80 | 参考 | 参考値 |
| P2 | **リンク失敗** | 約 +8,650 | 参考 | — | — | 参考 | FLASH 超過 |

### 読み取れること

- **フル機能（構成 E）で +6,784 B。** 16KB のうち base が 5,892 B を食っているので、
  残り 10,492 B に対して 65%。**載る。**
- **いちばん高いのは構成 C（全プリミティブ）で +2,892 B。** 使わない人はここを払わない。
- **文字は +1,284 B**（データ 216 B、コード 1,068 B）。
...
- **`pushImage` は +620 B。**
- **TileCanvas は +984 B / RAM +556 B。**
- **`Print`（float なし）は +280 B**（P1 − D）。
