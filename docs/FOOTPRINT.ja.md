...
| **D** | C + `setFont` / `drawString`（TinyFont 固定ピッチ・連続、フォントデータ 184 B を含む） |
...
| D | 11,620 | +5,728 | 6,400 | 576 | +72 | 96 | OK |
| E | 12,244 | **+6,352** | 7,200 | 576 | +72 | 96 | OK |
| T | 13,260 | +7,368 | 8,400 | 1,136 | +632 | 700 | OK |
| P1 | 11,880 | +5,988 | 参考 | 584 | +80 | 参考 | 参考値 |
| P2 | **リンク失敗** | 約 +8,648 | 参考 | — | — | 参考 | **FLASH を 4,144 B 超過** |

### 読み取れること

- **フル機能（構成 E）で +6,352 B。** 16KB のうち base が 5,892 B を食っているので、
  残り 10,492 B に対して 61%。**載る。**
- **いちばん高いのは構成 C（全プリミティブ）で +2,892 B。** 円・角丸・三角で
  ライブラリのコードの約 46% を使っている。使わない人はここを払わない
  （`linkprune` で確認済み）。
- **文字は +844 B**（うちフォントデータ 184 B、コード 660 B）。
  TinyFont に変えて **GFXfont のときより 236 B 減った**（[FONT_FORMAT.ja.md](FONT_FORMAT.ja.md) §4）。
- **`pushImage` は +624 B。**
- **TileCanvas は +1,016 B / RAM +560 B。** ちらつき対策の値札としては妥当。
  RAM は帯バッファ（240×1 行 = 480 B）が支配的で、幅と行数で線形に決まる。
- **`Print`（float なし）は +260 B**（P1 − D）。安い。
- **`println(float)` は約 8,648 B。** 11,880 B の P1 に足して 4,144 B 溢れたので、
  増分は 4,144 + (16,384 − 11,880) ≒ **8,648 B**。
