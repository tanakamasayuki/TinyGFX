...
| --- | --- | --- | --- | --- | --- | --- | --- |
| base | 5,892 | — | — | 504 | — | — | — |
| A | 7,604 | **+1,712** | 1,900 | 572 | +68 | 96 | OK |
| B | 7,880 | +1,988 | 2,200 | 572 | +68 | 96 | OK |
| C | 10,772 | +4,880 | 5,200 | 572 | +68 | 96 | OK |
| D | 11,808 | +5,916 | 6,400 | 572 | +68 | 96 | OK |
| E | 12,428 | **+6,536** | 7,200 | 572 | +68 | 96 | OK |
| T | 13,416 | +7,524 | 8,400 | 1,128 | +624 | 700 | OK |
| P1 | 12,092 | +6,200 | 参考 | 584 | +80 | 参考 | 参考値 |
| P2 | **リンク失敗** | 約 +8,650 | 参考 | — | — | 参考 | FLASH 超過 |

### 読み取れること

- **フル機能（構成 E）で +6,536 B。** 16KB のうち base が 5,892 B を食っているので、
  残り 10,492 B に対して 62%。**載る。**
- **いちばん高いのは構成 C（全プリミティブ）で +2,892 B。** 使わない人はここを払わない。
- **文字は +1,036 B**（データ 208 B、コード 828 B）。うち 144 B は
  **フォント形式を差し替えられるようにした代金**（[FONT_FORMAT.ja.md](FONT_FORMAT.ja.md) §8）。
  スイッチで 116 B 取り戻せる。
- **使っていないフォント形式は 0 バイト。** u8g2 のデコーダを持つ TinyGFX でも、
  TinyFont しか使わないスケッチには 1 バイトも載らない（`tests/linkprune/` が検査）。
- **`pushImage` は +620 B。**
- **TileCanvas は +988 B / RAM +556 B。**
- **`Print`（float なし）は +284 B**（P1 − D）。
...
- 2026-08-27 フォントを TinyFont に変えて D 以降が 236 B 減った。**予算は据え置き**
  （余裕ができただけ）。内訳は [FONT_FORMAT.ja.md](FONT_FORMAT.ja.md) §4。
