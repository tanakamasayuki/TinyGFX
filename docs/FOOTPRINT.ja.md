...
| --- | --- | --- | --- | --- |
| A 相当（`fillScreen`） | 7,240 | 1,612 | 4,376 | 1,377 |
| C 相当（全プリミティブ） | 10,444 | 1,612 | 7,076 | 1,387 |
| D 相当（+文字） | **11,476** | **1,612** | 8,532 | 1,427 |
| （参考）SPI + ST7789 の D | 11,808 | 572 | 7,228 | 150 |

- **フラッシュは SPI 版より 332 バイト小さい。** 初期化列が短く、画素の転送が
  バッファへのビット書き込みで済むため
- **RAM は +1,040 バイト**（フレームバッファ 1,024 + Wire の分）
- CH32V003 で 1,612 / 2,048 = **79%。載るが残り 436 バイト**
- Uno R3 で 1,427 / 2,048 = 70%
