...
**全部リリース版ツールの生成物**（2026-09-01。試作の `tools/img2h.py` は削除した）。

| プロジェクト | 中身 |
| --- | --- |
| `tests/image_oracle/` | ツールの出力を、ツール自身の画素と突き合わせる |
| `tests/image_fmt/` | 1 つの絵を符号化ごとに 1 枚。5 つのデコーダを走らせる |
| `tests/sh1106/` | ページ方式パネルへ流す splash（縦詰め 1bpp） |

`tests/regen_images.py` が 3 つとも作り直す。`--check` は commit 済みの古さを
落とす（npx が無ければ終了コード 3 で skip）。**元画像も `images/` に入って
いるので、誰でも作り直せる。**
