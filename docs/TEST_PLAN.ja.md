...
```text
TinyGFX → DriverST7789 → 本番の Bus → 線
...
| `ili9341/` | **ILI9341 と、DCS 系 3 パネルの init 列。** ILI9341 は `DriverDcs` からの継承が 21 行あるだけで、ILI9342C との違いは「GRAM が縦長・**反転が切**・**X ミラー付き**」の 3 点。**その 3 点はすべて `init()` が流すコマンド列に出る**のに、init 列を見ているテストが 1 つも無かった（`BusCapture` は 0x2A/0x2B/0x2C 以外を黙って捨てるので、SLPOUT を落としても COLMOD を変えても全部通っていた）。ST7789 / ILI9341 / ILI9342 を同じバスに流し、**並びごと**固定して「違うのは反転と MADCTL の 2 か所だけ」を検査する。回転 0 が **0x48**（他ライブラリがこの部品に送る値）であること、`setMirror(false,false)` / `setRgbOrder(false)` で外せること。**反転を入にする・ミラーを外す・SLPOUT を落とす、の 3 通りで落ちることを確認済み** | 通過 |
...
| `textwrap/` | **右端での自動折り返し**（`TINYGFX_TEXT_WRAP=1`。既定では入っていない）。折り返す位置、折り返し先が `setCursor` の x であること、`'\n'` がそのままであること。**幅の違う字**（数字は送り 4、'℃' は 8）で踏むので、送り幅を固定値で見ていたら落ちる。**固定値にする・境界を `>=` にする、の 2 通りで落ちることを確認済み** | 通過 |
| `panels/` | **パネルのカタログ**（`src/TinyGFX/panels/`）。プリセットが実際に運んでいるのは導出できない値だけなので、見るのはそこ —— **COM ピン配置**（ガラスの配線という事実）と**列オフセット**（中央でない品種）。128x32 を使うのは、**データシートのリセット値と違う唯一の品種**だから。仕組みが何も運んでいなければここで落ちる。あわせて `kBufferBytes` が正しいこと、**別ドライバのパネルが共存できること**（ガードはドライバ単位）、**240x240 ST7789 の回転 2 が GRAM の 80 行目から始まること**（`setGramSize` が効いている証拠。この repository の example は全部これを忘れていた）。**プリセットの値を消すと 2 通りとも落ちることを確認済み** | 通過 |
| `panels_seqcom/` | **`_SeqCom` の検査。同じドライバ・同じ寸法で値だけ違う**ケース。128x64 は COM 線の配線が両方流通しているのでカタログに 2 枚あり、**寸法では見分けられない** —— 高さからの導出が間違える唯一の形。多重比 0x3F（高さ 64 から導出）で寸法が同じことを確かめたうえで、COM ピンが既定の 0x12 ではなく **0x02** であることを見る。1 スケッチに 1 パネルしか入らないので `panels/` とは別スケッチ。**定義を消すと 0x12 になって落ちることを確認済み** | 通過 |
...
| `image_fmt/` | **同じ絵をどの形式で符号化しても 1 画素も違わないこと。** 生 RGB565 / RLE / RLE+パレット / 1bpp 横 / 1bpp 縦。**どれが選ばれたかがスケッチから見えてはいけない**（`tests/text/` の「3 通りの符号化が同じ画素を描く」と同じ考え方）。クリップ、画面外、透過も。**`raw565` は 1 行に窓を 1 つしか開かないこと** —— 写真は連長が 1 なので、連ごとに `fillRect` を呼ぶと画素ごとに窓が開き、1 画素 13 バイトになる（実測 3,072 コマンド／1,024 画素）。窓の数が行数と一致すること、はみ出しとクリップでも画面外に書かないことを固定。**元の実装に戻すと落ちることを確認済み。** **`dataLen` を 0 にして描き直す検査も持つ** —— raw565 と bitmap1h は 0 画素差、rle565 は 616 画素差。**どの形式に 65,535 バイトの上限が掛かるか**を決めているのがこれ（[IMAGE_FORMAT.ja.md](IMAGE_FORMAT.ja.md)） | 通過 |
| `image_oracle/` | **変換ツールの出力を TinyGFX が描いて突き合わせる。** 構成はツールのプロジェクト構成そのもの —— `images/` に元画像と `.imagesconfig`、その隣に `images.h`。**利用者のスケッチと同じ並び**なので、検査している include が利用者の書く include と同じになる。`regen.py` を 1 回走らせると比較の両側（`images.h` と `expected/*.png`）が同時に出る。収集時に `gen_sketch.py` がスケッチを組み立てる。**画像を足すのにコードは書かない。** **2026-08-31、`--preview` の出力を期待画像にする形に組み直し、5 デコーダ全部・11 枚が通過**（透過・**ディザ**・**減色**・**8 の倍数でない寸法**を含む。31x17 は 3 バイト+7 ビット、13x13 は 1 ページ+5 行 —— 行末の端数バイトで踏み外さないこと）。**透過を持つ絵はマゼンタの上に描く** —— 黒の上では、透過を無視するデコーダでも一致してしまうため。**ディザと減色は変換元から再現できない** —— 誤差拡散の実装とどの 16 色を残すかの選択はツールの側にしかない。**1 バイト壊すと 8/4096 画素、パレット 1 色をずらすと 128/2304 画素、`hasTransparent` を落とすと 268/576 画素の差を検出することを確認済み。** `regen.py --check` が committed の出力の古さを落とす。**ツールは `npx --yes gfx-image-tool@1.0.0` で呼ぶ。install はしない** —— バージョンがコマンドの中にあるのでずれようがない。別のバージョンで符号化が変わると `images.h mismatch` になり、元画像の差し替えと見分けが付かないため。npx が無ければ `regen.py` が終了コード 3 を返して skip | 通過 |
...
- **`TinyGFXMemoryTarget`** — RAM バッファへ直接書くパネル。`TileCanvas` の帯バッファと
