...
| `image_oracle/` | **変換ツールの出力を TinyGFX が描いて突き合わせる。** `pairs/` に `<名前>.h` と `<名前>.ppm` を置くと、収集時に `gen_sketch.py` がスケッチを組み立てて描き、比較する。**ペアを足すのにコードは書かない。** **2026-08-30、本番の CLI（GfxImageToolJs）の出力で 5 形式が通過**（raw565 / rlepal4 ×3 / bitmap1h / bitmap1v。透過つきを含む）。**1 バイト壊すと 130/1024 画素の差を検出することを確認済み。** 期待画像はいまのところ**変換元から作っている** —— RGB888→RGB565 も 1bpp の閾値も決定的なので符号化器を再実装せずに済む。**ディザと減色だけはこの方法が使えず**、ツール側の出力待ち（[EXTERNAL_REQUESTS.ja.md](EXTERNAL_REQUESTS.ja.md) E10） | 通過 |
