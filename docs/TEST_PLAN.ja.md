...
| `image_fmt/` | **同じ絵をどの形式で符号化しても 1 画素も違わないこと。** 生 RGB565 / RLE / RLE+パレット / 1bpp 横 / 1bpp 縦。**どれが選ばれたかがスケッチから見えてはいけない**（`tests/text/` の「3 通りの符号化が同じ画素を描く」と同じ考え方）。クリップ、画面外、透過も | 通過 |
