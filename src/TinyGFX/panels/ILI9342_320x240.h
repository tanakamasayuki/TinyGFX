// TinyGFXPanelILI9342_320x240
//
// M5Stack Core and BASIC. Landscape memory from the start, so
// nothing is offset. Older BASIC units come up inverted;
// invertDisplay(false) after begin() is the fix.
//
// Generated by tools/gen_panels.py - edit the catalogue there, not this file.
// If the picture is wrong, docs/PANEL_TUNING.ja.md says which line to change.
#pragma once
#ifdef TINYGFX_DRIVER_ILI9342_INCLUDED
#error "TinyGFX: one ILI9342 panel per sketch. Include this header rather than <TinyGFX/DriverILI9342.h>, and do not include a second panels/ILI9342_*.h - the second would be silently ignored and its panel driven with the first one's values."
#endif
#include "../DriverILI9342.h"

class TinyGFXPanelILI9342_320x240 : public TinyGFXDriverILI9342 {
 public:
  static const int16_t kWidth = 320;
  static const int16_t kHeight = 240;

  explicit TinyGFXPanelILI9342_320x240(TinyGFXBus& bus, int8_t rst = -1)
      : TinyGFXDriverILI9342(bus, kWidth, kHeight, rst) {}
};

// The name of the panel this sketch drives, so that it is written once:
//
//   TINYGFX_PANEL panel(bus);
//
// **Two panels and this disappears** - the second header to arrive removes it,
// because there is then no such thing as "the" panel. Name them explicitly.
#ifdef TINYGFX_PANEL_CLAIMED
#undef TINYGFX_PANEL
#else
#define TINYGFX_PANEL_CLAIMED 1
#define TINYGFX_PANEL TinyGFXPanelILI9342_320x240
#endif
