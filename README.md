...
| Panel | **the list below** (7 drivers, 24 panels) | `TinyGFX/panels/…` |
...
**A panel is a preset.** Include the one you bought and the dimensions, offsets
and COM wiring come with it.
...
| --- | --- | --- |
| **ST7789** | Colour (RGB565) | `240x240` / `135x240` / `240x320` / `240x280` / `170x320` / `172x320` |
| **ST7735** | Colour (RGB565) | `128x160` / `128x160 (FlushRgb)` / `128x128` / `80x160` / `80x160 (Offset24)` |
| **ST7796** | Colour (RGB565) | `320x480` |
| **ILI9341** | Colour (RGB565) | `240x320` |
| **ILI9342C** | Colour (RGB565) | `320x240` |
| **SSD1306** | Monochrome (1bpp) | `128x64` / `128x64 (SeqCom)` / `128x32` / `96x16` / `96x40` / `72x40` / `64x48` / `64x32` / `48x64` |
| **SH1106** | Monochrome (1bpp) | `128x64` |
<!-- END PANEL TABLE -->

`SeqCom`, `FlushRgb` and `Offset24` mark entries that **share a driver and a size
but differ inside**. The difference is the name, so you can pick one without
knowing what any other library calls it - no tab colours here.

### Some things a different include will not fix

**Picking your panel from the list settles the size, the offsets and the COM
wiring.** What it cannot settle is below, because **it varies between units and
production runs of the same product** - so no catalogue can hold it.

| | The fix | Where it goes |
| --- | --- | --- |
| **Black and white are swapped** | `panel.invertDisplay(false);` | **after `begin()`** - it puts a command straight on the bus |
| **Mirrored** (colour) | `panel.setMirror(mx, my);` | anywhere |
| **Red and blue swapped** (colour) | `panel.setRgbOrder(false);` | anywhere |
| **Shifted a few pixels** | `panel.setColumnOffset(n);` / `setOffset(x, y);` | anywhere |

There is a real example of this. **The M5Stack BASIC is the same ILI9342C
throughout, and only the older units come up inverted.** The panel type does not
decide it, so one line of `invertDisplay(false)` settles it.

### Not in the list, or still wrong

There are three kinds of fix.

| | What it takes |
| --- | --- |
| Orientation, inversion, colour order, offset | **one runtime call** (the table above) |
| **COM pin layout** (every other row a stripe) | **switch the include to the `_SeqCom` entry**, or `#define TINYGFX_SSD1306_COM_PINS 0x02` before it. Not changeable at runtime |
| **Pre-charge, VCOMH** (dim or flickering) | two `#define`s before the include. Not changeable at runtime |

The symptom-by-symptom table is in
[docs/PANEL_TUNING.md](docs/PANEL_TUNING.md). Freezing a setting you found into
the catalogue is one line in [`tools/gen_panels.py`](tools/gen_panels.py).

**This list is generated by `tools/gen_panels.py`.** Editing it by hand is undone
by the next run, and a test says so.
